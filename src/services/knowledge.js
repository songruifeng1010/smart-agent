const { getRandomResponse, calculateMatchScore } = require('../../utils');
...
class KnowledgeManager {
   ...
   */
  getKnowledgeResponse(cleanPrompt, chatModes, chatMode, userName) {
    // 检测对话模式切换
    if (cleanPrompt.includes('切换到') && (cleanPrompt.includes('模式') || cleanPrompt.includes('mode'))) {
      if (cleanPrompt.includes('友好')) {
        return { response: `已切换到${chatModes.friendly.name}！`, skipCache: false };
      } else if (cleanPrompt.includes('专业')) {
        return { response: `已切换到${chatModes.professional.name}！`, skipCache: false };
      } else if (cleanPrompt.includes('casual')) {
        return { response: `已切换到${chatModes.casual.name}！`, skipCache: false };
      } else if (cleanPrompt.includes('默认')) {
        return { response: `已切换到${chatModes.default.name}！`, skipCache: false };
      }
    }
    ...
    // 基础知识库
    const baseKnowledge = this.getBaseKnowledge(userName, chatModes[chatMode].name);
    ...
    if (bestBaseMatch) {
      ...
      if (response.includes('{{chatMode}}')) {
        response = response.replace('{{chatMode}}', chatModes[chatMode].name);
      }
   ...
   * 获取基础知识库
   * @param {string} userName - 用户名
   * @param {string} currentChatMode - 当前对话模式名称
   * @returns {object} 基础知识库
   */
  getBaseKnowledge(userName, currentChatMode) {
    return {
      greetings: {
        ...
        responses: [
          ...
          userName ? `${userName}你好！今天想聊点什么？` : '你好！想聊点什么吗？'
        ]
      },
      selfIntro: {
      ...
      chatMode: {
        ...
        responses: [
          ...
          `当前对话模式：${currentChatMode}。你可以随时切换其他模式哦！`
        ]
      },
      earth: {
   ...
   * @param {function} extractLearningKeyword - 关键词提取函数
   * @returns {string|null} 学习结果
   */
  learnFromUser(prompt, extractLearningKeyword) {
    // 解析用户输入，提取知识点和回答
    // 格式示例: "记住，苹果是红色的"
    // 格式示例: "学习，地球是圆的"
   ...
   */
  clearRelatedCache(cache, keyword) {
    if (cache) {
